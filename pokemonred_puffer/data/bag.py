...
class BagItem(LittleEndianStructure):
    _pack_ = 1
    _fields_ = [("Item", c_uint8), ("Quantity", c_uint8)]


class Bag(Union):
    _pack_ = 1
    _fields_ = [("bag", BAG_CAPACITY * BagItem), ("asbytes", c_uint8 * BAG_LENGTH_BYTES)]

    def __init__(self, emu: PyBoy):
        _, self.wBagItems = emu.symbol_lookup("wBagItems")
        _, self.wNumBagItems = emu.symbol_lookup("wNumBagItems")
